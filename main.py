    ...
    if wants_voice:
        ...
        if voice_file:
            if not is_private:
                ...
                # ИСПРАВЛЕНО: Убран reply_markup из одиночных ответов
...
async def global_errors_handler(event: ErrorEvent):
    ...
    update = event.update
    if update.message:
        # ИСПРАВЛЕНО: Убрана клавиатура из системных ошибок
...
async def start_command(message: Message):
    welcome_text = (
        ...
        "💬 <b>Живое общение</b> — просто пиши мне любой текст, я отвечу в своём стиле.\n"
        "🎙 <b>Голосовые ответы</b> — начни текст со слова <code>АУДИО</code>, чтобы я наговорил тебе ответ (лимит: 1 раз в 30 минут).\n"
        "🖼 <b>Разбор картинок</b> — скидывай фото, я оценю обстановку и выдам базу.\n"
        "🎛 <b>Меню действий</b> — введи команду /actions, чтобы вызвать панель с интерактивными кнопками.\n\n"
        "Полный список всех фич, лимитов и секретных команд всегда доступен по команде /help !"
    )
...
async def help_command(message: Message):
    help_text = (
        ...
        "<b>Базовые команды:</b>\n"
        "/start — Перезапустить бота, вывести приветствие и краткие инструкции.\n"
        "/actions — Вызвать панель управления (новости, сливы, мерч, ЕСПЧ, сброс памяти).\n"
        "/help — Открыть это подробное руководство.\n\n"
        "<b>Мультимедийные фичи:</b>\n"
        "• 🎧 <b>Голос в текст:</b> Отправляй мне свои голосовые сообщения или аудиофайлы. Я использую Gemini, чтобы дословно их расшифровать, а затем хлёстко ответить.\n"
        "• 🗣 <b>Текст в голос:</b> Напиши сообщение в формате: <code>АУДИО твой текст</code>. Я отправлю в ответ полноценное аудиосообщение со своим оригинальным голосом (из-за дефицита бюджетов ФБК функция имеет лимит: 1 раз в 30 мин для обычных пользователей).\n"
        "• 👁 <b>Компьютерное зрение:</b> Скидывай фотографии (можно с текстовым описанием). Я проведу сатирический разбор происходящего на картинке, узнаю известных политиков или прочитаю текст.\n\n"
        "<b>Суд Люстрации (Для групповых чатов):</b>\n"
        "• /sud — Добавь меня в группу. Отправь эту команду <i>в ответ (reply)</i> на сообщение любого участника чата. Я выступлю судьёй Комитета Люстрации и вынесу жулику уникальный приговор.\n\n"
        "<b>Инлайн-режим (Работает в ЛЮБЫХ чатах):</b>\n"
        "• В поле ввода абсолютно любого диалога в Telegram набери: <code>@navalniy_chat_bot [твой текст]</code>. На выбор появятся быстрые пресеты (сливы, мерч) или динамическая генерация ответа на твой вопрос прямо внутри чужого чата!"
    )
    await message.answer(help_text)

@dp.message(Command("actions"))
async def actions_command(message: Message):
    # ОСТАВЛЕНО: Кнопки создаются строго по этой команде
...
async def reset_handler(callback: CallbackQuery):
    ...
    # ОСТАВЛЕНО: Возвращаем меню действий после очистки, это удобно
